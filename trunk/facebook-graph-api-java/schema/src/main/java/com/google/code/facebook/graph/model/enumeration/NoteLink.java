...
/**
 * The Enum NoteConnection.
 */
public enum NoteLink implements ConnectionEnum {
	
	/** The FROM. */
	FROM("from", User.class);

    /** The Constant stringToEnum. */
	private static final Map<String, NoteLink> stringToEnum = new HashMap<String, NoteLink>();

    /** The Constant weightsMap. */
	private static final Map<NoteLink, Double> weightsMap = new EnumMap<NoteLink, Double>(NoteLink.class);
	
	static { // Initialize map from constant name to enum constant
		for (NoteLink op : values()) {
    ...
    /**
     * Instantiates a new note connection.
     * 
     * @param value the value
     * @param type the type
     */
    NoteLink(String value, Class<? extends Entity<?, ?>> type) {
        this.value = value;
        this.type = type;
    }
	 ...
	 * @param value the value
	 * 
	 * @return the note connection
	 */
	public static NoteLink fromValue(String value) {
